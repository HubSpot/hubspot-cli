...
function debounceQueueBuild(
  ...
  timer = setTimeout(async () => {
    logger.debug(
      i18n(`commands.project.subcommands.watch.debug.pause`, { projectName })
    );
    queue.pause();
    await queue.onIdle();
    ...
    try {
      ...
      logger.debug(
        i18n(`commands.project.subcommands.watch.debug.buildStarted`, {
          projectName,
        })
      );
    } catch (err) {
      if (
        ...
        logger.log(
          i18n(`commands.project.subcommands.watch.logs.watchCancelledFromUi`)
        );
        process.exit(0);
      } else {
...
async function queueFileOrFolder(
  ...
  if (
    ...
    logger.debug(
      i18n(`commands.project.subcommands.watch.debug.extensionNotAllowed`, {
        filePath,
      })
    );
    return;
  ...
  if (shouldIgnoreFile(filePath, true)) {
    logger.debug(
      i18n(`commands.project.subcommands.watch.debug.ignored`, { filePath })
    );
    return;
  }
  if (!queue.isPaused) {
  ...
  logger.debug(
    i18n(`commands.project.subcommands.watch.debug.uploading`, {
      filePath,
      remotePath,
    })
  );
  ...
  return queue.add(async () => {
    try {
      ...
      logger.log(
        i18n(`commands.project.subcommands.watch.logs.${action}Succeeded`, {
          filePath,
          remotePath,
        })
      );
      ...
      logger.debug(
        i18n(`commands.project.subcommands.watch.errors.${action}Failed`, {
          filePath,
          remotePath,
        })
      );
...
async function createNewBuild(
  ...
  try {
    logger.debug(
      i18n(`commands.project.subcommands.watch.debug.attemptNewBuild`)
    );
    const {
      data: { buildId },
    ...
    if (
      ...
      logger.log(
        i18n(
          `commands.project.subcommands.watch.logs.previousStagingBuildCancelled`
        )
      );
    }
    process.exit(1);
...
async function handleWatchEvent(
  ...
  if (queue.isPaused) {
    if (standbyQueue.find(file => file.filePath === filePath)) {
      logger.debug(
        i18n(`commands.project.subcommands.watch.debug.fileAlreadyQueued`, {
          filePath,
        })
      );
    } else {
...
export async function createWatcher(
  ...
  watcher.on('ready', async () => {
    logger.log(
      i18n(`commands.project.subcommands.watch.logs.watching`, { projectDir })
    );
    logger.log(`\n> Press ${chalk.bold('q')} to quit watching\n`);
  });
