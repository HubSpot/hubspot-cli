...
import { Mode, MODULE_EXTENSION, FUNCTIONS_EXTENSION } from './lib/constants';
 ...
 * @param {Mode} mode
 */
export function getFileMapperApiQueryFromMode(mode) {
  return {
 ...
 */
export async function fetchFolderFromApi(input) {
 ...
 * @returns {Promise}
 */
export async function downloadFolder(input) {
  try {
 ...
 * @returns {Promise}
 */
export async function downloadFileOrFolder(input) {
  try {
    if (!(input && input.src)) {
      return;
    }
