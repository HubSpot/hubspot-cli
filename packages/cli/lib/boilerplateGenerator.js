...
async function downloadFileAsBuffer(url) {
  ...
  if (response.ok) {
    ...
    throw new Error(
      `Could not download code from ${url}: ${response.status} ${response.statusText}`
    );
  }
}
...
async function downloadGithubRepo(url) {
  ...
  const files = await downloadBoilerplateComponent(
    owner,
    repo,
    filePath,
    branch
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'hs-project-generate-' + Date.now())
  );
