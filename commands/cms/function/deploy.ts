...
export type FunctionDeployArgs = CommonArgs &
  ConfigArgs &
  AccountArgs &
...
async function handler(
  ...
  if (
    ...
    uiLogger.error(
      commands.cms.subcommands.function.subcommands.deploy.errors.notFunctionsFolder(
        functionPath
      )
    );
    return;
  ...
  uiLogger.debug(
    commands.cms.subcommands.function.subcommands.deploy.debug.startingBuildAndDeploy(
      functionPath
    )
  );
  ...
  try {
    ...
    if (successResp) {
      ...
      uiLogger.success(
        commands.cms.subcommands.function.subcommands.deploy.success.deployed(
          functionPath,
          derivedAccountId,
          buildTimeSeconds
        )
    ...
    SpinniesManager.fail('loading', {
      text: commands.cms.subcommands.function.subcommands.deploy.loadingFailed(
        functionPath,
        uiAccountDescription(derivedAccountId)
      ),
    });
    ...
    if (isHubSpotHttpError(e) && e.status === 404) {
      uiLogger.error(
        commands.cms.subcommands.function.subcommands.deploy.errors.noPackageJson(
          functionPath
        )
      );
      ...
      uiLogger.error(
        commands.cms.subcommands.function.subcommands.deploy.errors.buildError(
